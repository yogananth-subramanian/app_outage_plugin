...
def _setup_kubernetes(kubeconfig) -> client.ApiClient:
    """
    Sets up the Kubernetes client
    """
    kubeconfig_dict = yaml.safe_load(kubeconfig)
    client_config = config.load_kube_config_from_dict(kubeconfig_dict)
    return client.ApiClient(client_config)

def setup_kubernetes(kubeconfig) ->client.ApiClient:
...
def create_ifb(cli, number, pod_name):
    ...
    exec_command = ['chroot', '/host', 'modprobe',
                    'ifb', 'numifbs=' + str(number)]
...
def list_net_policy(cli, namespace, label_selector=None):
    """
    Function used to list network policy in a given namespace and having a certain label
    """

    policies = []
    try:
        if label_selector:
            ret = cli.list_namespaced_network_policy(
                namespace, pretty=True, label_selector=label_selector)
        else:
            ret = cli.list_namespaced_network_policy(namespace, pretty=True)
    except ApiException as e:
        logging.error(
            "Exception when calling \
                       CoreV1Api->list_namespaced_pod: %s\n"
        ...
        raise e
    for policy in ret.items:
        policies.append(policy.metadata.name)

    return policies


def read_net_policy(cli, name, namespace="default"):
    """
    Function that retrieves the info of a network policy in a given namespace
    """

    return cli.read_namespaced_network_policy(name=name, namespace=namespace)


def list_service(cli, namespace, label_selector=None):
    """
    Function used to list service in a given namespace and having a certain label
    """

    services = []
    try:
        if label_selector:
            ret = cli.list_namespaced_service(
                namespace, pretty=True, label_selector=label_selector)
        else:
            ret = cli.list_namespaced_service(namespace, pretty=True)
    except ApiException as e:
        logging.error(
            "Exception when calling \
                       CoreV1Api->list_namespaced_pod: %s\n"
    ...
    for service in ret.items:
        services.append(service.metadata.name)

    return services


def read_service(cli, name, namespace="default"):
    """
    Function that retrieves the info of a service in a given namespace
    """

    return cli.read_namespaced_service(name=name, namespace=namespace)
...
def list_crd(cli):
    ...
    crds = []
    try:
        ret = cli.list_custom_resource_definition()
    except ApiException as e:
        logging.error(
            "Exception when calling \
                       CoreV1Api->list_namespaced_pod: %s\n"
